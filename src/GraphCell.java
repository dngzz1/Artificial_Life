...
class GraphCell extends Cell {
	...
	static GraphCell cloneAndMutate(GraphCell cellToClone){
		int i = 0;
		GraphCell cloneCell = new GraphCell(cellToClone);
	...
	GraphCell(LinkedList<String> lineList){
	...
	private void setupOrgans(){
		...
		Organ_Reproduction defaultReproductionOrgan = new Organ_Reproduction(GraphCell.energyUponBirth);
	...
	void spawn(Direction direction, int energyPassedToChild){
		...
		if(energy > energyCost){
			...
			GraphCell child = cloneAndMutate(this);
